...
try {
    ...
    if ($action === 'list_proveedores') {
        ...
        $stmt = $pdo->query("SELECT p.id, p.sku, p.nombre, p.marca, p.tipo,
        ...
        // Full transaction: record movement + update inventory + optionally update price
        ...
        foreach ($items as $item) {
            ...
            // 1. Registro de movimiento de inventario_ICA_final
            $stmt = $pdo->prepare("INSERT INTO inventario_movimiento_ICA_final 
                (producto_id, tipo, cantidad, fecha, proveedor_id)
                VALUES (?, 'RECEPCION', ?, NOW(), ?)");
            $stmt->execute([$producto_id, $cantidad, $proveedor_id]);

            // 2. Actualizar o insertar en inventario_ICA_final (zona específica)
            $check = $pdo->prepare("SELECT id, cantidad FROM inventario_ICA_final 
                                    WHERE producto_id=? AND zona_id=? AND es_reserva=?");
            $check->execute([$producto_id, $zona_id, $es_reserva]);
            $existing = $check->fetch(PDO::FETCH_ASSOC);

            if ($existing) {
                $upd = $pdo->prepare("UPDATE inventario_ICA_final SET cantidad = cantidad + ? WHERE id = ?");
                $upd->execute([$cantidad, $existing['id']]);
            } else {
                $ins = $pdo->prepare("INSERT INTO inventario_ICA_final (producto_id, zona_id, cantidad, es_reserva) VALUES (?,?,?,?)");
                $ins->execute([$producto_id, $zona_id, $cantidad, $es_reserva]);
            }

            // 3. Actualizar precio si se proporcionó
            if ($precio_nuevo && $precio_nuevo > 0) {
                $pdo->prepare("UPDATE lista_precio_ICA_final SET vigente=0 WHERE producto_id=?")->execute([$producto_id]);
                $pdo->prepare("INSERT INTO lista_precio_ICA_final (producto_id, precio, vigente, fecha) VALUES (?,?,1,NOW())")->execute([$producto_id, $precio_nuevo]);
            }
        }
